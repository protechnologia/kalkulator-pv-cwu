...
pv-sim.render.js      — fmt, smoothPath(), renderChart/Stats dla modułów 01–02 i 05–06
...
JS: `config` → `physics` → `optimize` → `render` → `render.m03` → `render.m04` → `render.m07` → `render.m08` → `app`
